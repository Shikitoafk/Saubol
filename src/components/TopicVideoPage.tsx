...
export default function TopicVideoPage({ video, onStartPractice }: TopicVideoPageProps) {
  return (
    <div className="max-w-6xl mx-auto p-12 selection:bg-indigo-500/30">
      ...
      <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-white/20 mb-12">
         <span>SAT Learn</span>
         <span className="opacity-40">/</span>
         <span className="text-white/60">{video.title}</span>
      </div>
      ...
      <header className="mb-12">
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/5">
             ...
             <span className="text-[9px] font-black uppercase text-white/40">{video.duration} Exploration</span>
          </div>
      ...
      <div className="aspect-video w-full glass-3d border-white/5 overflow-hidden mb-16 relative group">
        {video.videoId ? (
          ...
          <div className="w-full h-full bg-white/[0.02] flex flex-col items-center justify-center gap-6">
            ...
            <p className="text-xs font-black uppercase tracking-[0.4em] text-white/20">Video Coming Soon</p>
            <Button variant="outline" onClick={onStartPractice} className="border-white/10 text-white/40 uppercase font-black text-[10px] tracking-widest h-12 px-8 rounded-xl hover:bg-white/5">
      ...
      <div className="grid md:grid-cols-2 gap-16 items-start">
            ...
            <div className="space-y-4">
              {video.topics_covered.map((topic, i) => (
                <div key={i} className="flex items-center gap-4 group">
                  ...
                  <span className="text-lg font-medium text-white/60 group-hover:text-white transition-colors">{topic}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="glass-3d p-10 border-indigo-500/20 bg-indigo-500/5 relative overflow-hidden">
           ...
           <div className="relative z-10">
              ...
              <p className="text-white/60 text-sm leading-relaxed font-medium mb-8">
