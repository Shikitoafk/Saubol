...
export default function SATTests() {
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-canvas text-ink pt-32 pb-20 px-10 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="mb-20">
            ...
            <div className="flex gap-12 border-b border-line pb-4">
              <button onClick={() => setActiveTab('available')} className={`text-[11px] font-black uppercase tracking-widest transition-colors pb-4 relative ${activeTab === 'available' ? 'text-ink' : 'text-ink-subtle'}`}>
                Available Simulations
                {activeTab === 'available' && <motion.div layoutId="tab" className="absolute bottom-0 left-0 right-0 h-1 bg-white" />}
              </button>
              <button onClick={() => setActiveTab('completed')} className={`text-[11px] font-black uppercase tracking-widest transition-colors pb-4 relative ${activeTab === 'completed' ? 'text-ink' : 'text-ink-subtle'}`}>
                Test History
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            {testTypes.map((test, i) => (
              <motion.div
                ...
                className={`glass-3d p-12 flex flex-col justify-between group hover:border-line-strong transition-all ${test.border} ${test.bg}`}
              >
                <div>
                   <div className="flex justify-between items-start mb-12">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border ${test.border} bg-canvas/40`}>
                         <test.icon className={`w-7 h-7 ${test.color}`} />
                      </div>
                      <div className="text-right">
                         <p className="text-[9px] font-black text-ink-subtle uppercase tracking-widest">Format</p>
                         <p className="text-xs font-black uppercase text-ink-muted">{test.difficulty}</p>
                      </div>
                   </div>
                   
                   <h3 className="text-4xl font-black tracking-tighter uppercase italic mb-4">{test.title}</h3>
                   <p className="text-ink-muted font-medium text-sm leading-relaxed mb-10">{test.description}</p>
                   
                   <div className="grid grid-cols-2 gap-4 mb-12">
                      <div className="p-4 bg-canvas/40 rounded-xl border border-line">
                         <p className="text-[9px] font-black text-ink-subtle uppercase tracking-widest mb-1">Questions</p>
                         <p className="text-xl font-black">{test.questions}</p>
                      </div>
                      <div className="p-4 bg-canvas/40 rounded-xl border border-line">
                         <p className="text-[9px] font-black text-ink-subtle uppercase tracking-widest mb-1">Duration</p>
          ...
          <div className="mt-32 p-16 glass-3d border-indigo-500/10 bg-indigo-500/5">
             <div className="flex flex-col md:flex-row items-center justify-between gap-12">
                <div className="max-w-xl">
                   ...
                   <p className="text-lg text-ink-muted font-medium leading-relaxed">Based on your last 3 module tests, your estimated score trajectory is <span className="text-ink">1480-1540</span>. We recommend a Full Practice test to confirm timing resilience.</p>
                </div>
                <div className="flex gap-6">
                   <div className="text-center p-8 glass-3d border-line min-w-[160px]">
